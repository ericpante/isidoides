...
# 2ND RECALIBRATION 
# Get recalibration table after 2nd pass on unrecalibrated data

gatk BaseRecalibrator \
    -R ${REFERENCE} \
    -I ${INPUT_BAM} \
    --known-sites ${OUTDIR}/bqsr_snps_recal1.vcf \
    --known-sites ${OUTDIR}/bqsr_indels_recal1.vcf \
    -O ${OUTDIR}/${SAMPLE}_recal2.table
...
gatk ApplyBQSR \
    -R ${REFERENCE} \
    -I ${INPUT_BAM} \
    --bqsr-recal-file ${OUTDIR}/${SAMPLE}_recal2.table \
    -O ${OUTDIR}/${SAMPLE}_recal2.bams
...
gatk BaseRecalibrator \
    -R ${REFERENCE} \
    -I ${OUTDIR}/${SAMPLE}_recal2.bams \
    --known-sites ${OUTDIR}/bqsr_snps_recal1.vcf \
    --known-sites ${OUTDIR}/bqsr_indels_recal1.vcf \
    -O ${OUTDIR}/${SAMPLE}_post_recal2.table
