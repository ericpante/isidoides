...
cd $OUTDIR
find ${OUTDIR} -type f -name "*_recal1.g.vcf" > gvcf.list

# Consolidate all g.vcf files
gatk CombineGVCFs \
    -R ${REFERENCE} \
    -V ${OUTDIR}/gvcf.list \
    -O ${OUTDIR}/combined_gvcf_recal1.vcf

# Joint Genotypiing                                                                                                                                                           
gatk GenotypeGVCFs \
    -R ${REFERENCE} \
    -V ${OUTDIR}/combined_gvcf_recal1.vcf \
    -O ${OUTDIR}/genotyped_samples_recal1.vcf

# Extract SNPSs from callset
gatk SelectVariants \
	-R ${REFERENCE} \
	-V ${OUTDIR}/genotyped_samples_recal1.vcf \
	-select-type SNP \
	-O ${OUTDIR}/genotyped_samples_snps_recal1.vcf
...
gatk SelectVariants \
	...
	-V ${OUTDIR}/genotyped_samples_recal1.vcf \
	-select-type INDEL \
	-O ${OUTDIR}/genotyped_samples_indels_recal1.vcf
