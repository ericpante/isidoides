...
cd $OUTDIR
find ${OUTDIR} -type f -name "*g.vcf" > gvcf.list

# Consolidate all g.vcf files
gatk CombineGVCFs \
    -R ${REFERENCE} \
    -V ${OUTDIR}/gvcf.list \
    -O ${OUTDIR}/combined_gvcf.vcf

# Joint Genotypiing                                                                                                                                                           
gatk GenotypeGVCFs \
    -R ${REFERENCE} \
    -V ${OUTDIR}/combined_gvcf.vcf \
    -O ${OUTDIR}/genotyped_samples.vcf

# Extract SNPSs from callset
gatk SelectVariants \
	-R ${REFERENCE} \
	-V ${OUTDIR}/genotyped_samples.vcf \
	-select-type SNP \
	-O ${OUTDIR}/genotyped_samples_snps.vcf
...
gatk SelectVariants \
	...
	-V ${OUTDIR}/genotyped_samples.vcf \
	-select-type INDEL \
	-O ${OUTDIR}/genotyped_samples_indels.vcf
